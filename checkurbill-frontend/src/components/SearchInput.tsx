...
const SearchInput = (props: SearchInputProps) => {
  const [getSearchQuery, setSearchQuery] = useSearchParams();
  const [search, setSearch] = useState<string>(
    getSearchQuery.get("search") || ""
  );
  const [y, setY] = useState(window.scrollY);
  const onhandleSearchQuery = (e: React.SyntheticEvent) => {
    e.preventDefault();
    if (search === "") {
      setSearchQuery();
    } else {
      // if (getSearchQuery.get("page") && getSearchQuery.get("page") !== null) {
      //   setSearchQuery({ search, page: getSearchQuery.get("page")! });
      // } else {
  ...
  return (
    <>
    <form onSubmit={onhandleSearchQuery} className=" search-uput-form">
      <label
