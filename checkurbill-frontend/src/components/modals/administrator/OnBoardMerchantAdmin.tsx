...
import { useOnBoardAdminMerchantMutation } from "../../../services/administrator-service";
import Spinner from "../Spinner";
import { ErrorHandling } from "../../../helpers/errorHandling";

type OnBoardMerchantAdminProps = {
  modalCreateAdminMerchant: boolean;
  setCreateAdminMerchantModal: (
    open: boolean,
...
export interface AddMerchantAdminInterface {
  firstname: string;
  lastname: string;
  middlename: string;
  email: string;
...
const OnBoardMerchantAdmin = (props: OnBoardMerchantAdminProps) => {
  const {
    register,
    handleSubmit,
    watch,
    formState: { errors },
    reset,
  } = useForm<AddMerchantAdminInterface>({
    resolver: yupResolver(schema),
  });

  const [onBoardAdminMerchant, { isLoading }] =
    useOnBoardAdminMerchantMutation();

  const [image, setImage] = useState<null | string | ArrayBuffer>(null);

  const handleAddMerchantAdmin: SubmitHandler<
  ...
  > = async (formdata) => {
    try {
      ...
      let response = await onBoardAdminMerchant(formdata).unwrap();
      reset();
      props.setCreateAdminMerchantModal(false);
  ...
  return (
    <div
      ...
      aria-hidden="true"
      className={`${
        props.modalCreateAdminMerchant ? "" : "hidden"
        ...
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
          ...
          <div className="flex justify-between items-start p-4 rounded-t border-b dark:border-gray-600">
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
              Add New Administrator for Merchant:{" "}
            ...
            <button
              ...
              onClick={() => props.setCreateAdminMerchantModal(false)}
            >
              <svg
                aria-hidden="true"
                className="w-5 h-5"
          ...
          <div className="p-6 space-y-6">
            <form onSubmit={handleSubmit(handleAddMerchantAdmin)}>
...
export default OnBoardMerchantAdmin;
